  ...
  async generateQuery({ query }) {
    ...
    try {
      ...
      const vm = new VM({
        sandbox: {
          Sequelize,
          Op, // Sequelize operators
          require, // Allow importing other modules if needed
          console, // Optional: Allow console logging inside the VM
          User,
          File, 
          FileOwner, 
        },
      ...
      let aiQueryConfig;
      try {
        aiQueryConfig = vm.run(`(${aiQueryConfigRaw})`);
      } catch (error) {
